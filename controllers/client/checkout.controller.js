...
} = require("../../socket/admin/dashboard.socket");
...
module.exports.index = async (req, res) => {
  ...
  cart.products = cart.products.filter((item) =>
    listProductIds.includes(item.id)
  );
  ...
  if (cart.products.length > 0) {
    for (const item of cart.products) {
      ...
      item.productInfo.priceNewFormat = Number(
...
module.exports.confirmOrder = async (req, res) => {
  ...
  if (paymentType === "momo") {
    ...
    try {
      ...
      console.log("ZaloPay Query Response:", result.data);
      console.log("Created app_trans_id:", orders.app_trans_id);
      await Order.updateOne(
        { _id: order.id },
        { app_trans_id: orders.app_trans_id }
      );
...
module.exports.success = async (req, res) => {
  ...
  if (order.paymentType === "momo") {
    ...
    // Giả sử app_trans_id đã được lưu trong Order khi tạo giao dịch
    const app_trans_id = order.app_trans_id; // Cần lưu app_trans_id khi tạo đơn hàng
    // Trong phần kiểm tra trạng thái
    console.log("Checking app_trans_id:", app_trans_id); // Debug
    const rawSignature = `${config.app_id}|${app_trans_id}`;
    const mac = crypto
    ...
    try {
      ...
      const { return_code, message } = result.data;

      if (return_code != 1) {
        paymentStatus = "success";
        paymentMessage = "Thanh toán thành công qua ZaloPay.";
        await Order.updateOne(
          { _id: order.id },
          { paymentStatus: "completed" }
        );
        ...
        paymentStatus = "pending";
        paymentMessage = `Giao dịch đang chờ xử lý: ${message}`;
      }
    } catch (error) {
      paymentStatus = "error";
      paymentMessage =
        "Không thể kiểm tra trạng thái thanh toán ZaloPay. Vui lòng thử lại sau.";
    }
  } else {
